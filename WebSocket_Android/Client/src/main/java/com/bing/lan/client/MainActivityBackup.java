...
public class MainActivityBackup extends AppCompatActivity implements View.OnClickListener, SocketClient.SocketListener, CompoundButton.OnCheckedChangeListener {
    ...
    public void onClick(final View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.connect:
                if (mSocketClient != null) {
                    Toast.makeText(MainActivityBackup.this, "已连接", Toast.LENGTH_SHORT).show();
                    return;
                }
                mSocketClient = new SocketClient(MainActivityBackup.this);
                mSocketClient.setSocketListener(MainActivityBackup.this);
            ...
            case R.id.send:
                if (mSocketClient == null) {
                    Toast.makeText(MainActivityBackup.this, "请先连接服务器", Toast.LENGTH_SHORT).show();
                    return;
                }
                final String trim = mContent.getText().toString().trim();
                if (TextUtils.isEmpty(trim)) {
                    Toast.makeText(MainActivityBackup.this, "请输入内容", Toast.LENGTH_SHORT).show();
                    return;
                }
    ...
    public void onSendMessage(boolean isSuccess, Message clientMessage) {
        if (isSuccess) {
            Toast.makeText(MainActivityBackup.this, "发送成功", Toast.LENGTH_SHORT).show();
            mContent.setText("");
            mMessages.add(clientMessage);
            mAdapter.notifyDataSetChanged();
        } else {
            Toast.makeText(MainActivityBackup.this, "发送失败", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void onStopConnect() {
        Toast.makeText(MainActivityBackup.this, "连接断开", Toast.LENGTH_SHORT).show();
    }
